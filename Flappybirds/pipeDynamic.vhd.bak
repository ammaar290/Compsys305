...
size <= CONV_STD_LOGIC_VECTOR(200,20);  -- changing to a rectangular pipe spanning vertically
ball_y_pos <= CONV_STD_LOGIC_VECTOR(350,10);  -- keeping the pipe at the same vertical position

process(clk)
begin
	if rising_edge(clk) then
		if ball_x_pos = CONV_STD_LOGIC_VECTOR(0,10) then
			ball_x_pos <= CONV_STD_LOGIC_VECTOR(840,10);  -- reset position to the right corner when it reaches the left edge
		else
			ball_x_pos <= ball_x_pos - CONV_STD_LOGIC_VECTOR(1,10);  -- decrement the x-position by 1 in each clock cycle
		end if;
	end if;
end process;
